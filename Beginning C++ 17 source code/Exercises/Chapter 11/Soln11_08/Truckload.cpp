...
// Get the current Box
SharedBox Truckload::Iterator::getCurrentBox()    
{
  return pCurrent? pCurrent->pBox : nullptr;       // Return the current box (or nullptr...).
}
...
bool Truckload::removeBox(SharedBox boxToRemove)
{
  ...
  {
    if (box == boxToRemove)          // We found the Box!
    {
	  removeBox(iterator);
...
// Remove the Box that was last returned by the given Iterator
bool Truckload::removeBox(Iterator iterator)
{
  auto current = iterator.pCurrent;        // Extract a pointer to the current Package from the Iterator
  if (!current)
  {
    return false;
  }
